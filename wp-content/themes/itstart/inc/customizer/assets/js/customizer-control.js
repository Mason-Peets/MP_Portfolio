...
( function( $ ) {
	...
	ITSTART_Customizer = {
		...
		init: function()
		{
			ITSTART_Customizer._initToggles();
		},
		...
		_initToggles: function()
		{
			...
			ITSTART_ControlTrigger.triggerHook( 'itstart-toggle-control', api );

			// Loop through each setting.
			$.each( ITSTART_CustomizerToggles, function( settingId, toggles ) {
						...
						$.each(toggles, function( i, toggle) {
							...
							$.each(toggle.controls, function( k, controlId) {
								...
								if ( typeof parent != 'undefined' ) {
									...
									if ( ! visibleIt ) {
										$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'itstart-hide' );
									} else {
										$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'itstart-hide' );
									}
								}
							});
						});
	...
	$( function() { ITSTART_Customizer.init(); } );
